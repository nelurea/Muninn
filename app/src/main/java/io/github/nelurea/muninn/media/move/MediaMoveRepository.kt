...
interface MediaMovePersistence {
    ...
    suspend fun switchDatabase(journal: MediaMoveJournalEntity, now: Long): Boolean
...
class MediaMoveRepository(private val dao: MediaMoveDao) : MediaMovePersistence {
    ...
    override suspend fun switchDatabase(journal: MediaMoveJournalEntity, now: Long) = dao.switchDatabase(journal, now)
